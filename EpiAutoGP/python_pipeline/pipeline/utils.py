...
def build_pyrenew_hew_model_from_dir(
    ...
    """
    Build a PyRenew-HEW epidemiological model from configuration files.
    
    Constructs a PyRenew-HEW model by loading priors and parameters from either
    a model directory structure or explicit file paths. The model can be configured
    to fit different data streams (ED visits, hospital admissions, wastewater).

    Parameters
    ----------
    model_dir : Path | str, optional
        Directory containing model configuration files. If provided, expects:
        - priors.py: Python file defining model priors
        - data/model_params.json: JSON file with model parameters
    prior_path : Path | str, optional
        Explicit path to priors Python file (used if model_dir not provided)
    model_params_path : Path | str, optional
        Explicit path to model parameters JSON file (used if model_dir not provided)
    fit_ed_visits : bool, default False
        Whether to include emergency department visits in the model
    fit_hospital_admissions : bool, default False
        Whether to include hospital admissions in the model
    fit_wastewater : bool, default False
        Whether to include wastewater viral concentration in the model

    Returns
    -------
    PyrenewHEWModel
        Configured epidemiological model ready for fitting
    ...
    Notes
    -----
    - Either provide model_dir OR both prior_path and model_params_path
    - The priors file is executed to load RandomVariable objects
    - At least one data stream (ED visits, hospital admissions, or wastewater) 
      should typically be enabled for meaningful model fitting
    """
