...
def create_epiautogp_input(
    ...
    print(
        f"Filtered data shape for report_date {report_date}: {filtered_df.shape}"
    )
    ...
    print(
        f"Data contains {len(dates)} time points from {min(dates)} to {max(dates)}"
    )
    print(
        f"Nowcast created for {last_date} with {len(nowcast_samples)} samples"
    )
...
def main():
    ...
    print(
        f"Report values: min={min(result['reports'])}, max={max(result['reports'])}"
    )
    print(
        f"Nowcast dates: {len(result['nowcast_dates'])} ({result['nowcast_dates']})"
    )
