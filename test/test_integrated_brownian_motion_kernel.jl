@testitem "IntegratedBrownianMotion eval_cov (scalar form)" begin
    ...
    node = IntegratedBrownianMotion(2.0)
    # For s <= t the covariance is amplitude * (s^2 t / 2 - s^3 / 6).
    s, t = 0.3, 0.5
    ...
    @test IntegratedBrownianMotion().amplitude == 1
    @test fieldnames(IntegratedBrownianMotion) == (:amplitude,)
end
...
@testitem "IntegratedBrownianMotion eval_cov (matrix form) matches scalar and is PSD" begin
    ...
    node = IntegratedBrownianMotion(1.5)
...
@testitem "IntegratedBrownianMotion reparameterize matches input warping" begin
    ...
    # closed-form parameter: amplitude is degree-3 in slope
...
@testitem "IntegratedBrownianMotion pretty printing" begin
    ...
    @test AutoGP.GP.pretty(IntegratedBrownianMotion(2.0)) == "IBM(2.00)"
    @test AutoGP.GP.pretty(IntegratedBrownianMotion(0.5)) == "IBM(0.50)"
end
